...
def get_model_result(file_name):
    ...
    box4 = (135, 0, 180, 60)
    # 依次截取并转灰度
    image1 = convert_to_gray(np.array(image.crop(box1)))
    image2 = convert_to_gray(np.array(image.crop(box2)))
    image3 = convert_to_gray(np.array(image.crop(box3)))
    image4 = convert_to_gray(np.array(image.crop(box4)))
    # 转模型输入
    result = []
    input1 = image1.flatten() / 255
    input2 = image2.flatten() / 255
    input3 = image3.flatten() / 255
    input4 = image4.flatten() / 255
    # 计算
    X, conv = verify_code_cnn()
    saver = tf.train.Saver()
    with tf.Session() as sess:
        saver.restore(sess, 'model/verify_code.model')
        predict = tf.argmax(conv, 1)
        result.append(index_to_char(sess.run(predict, feed_dict={X: [input1]})[0]))
        result.append(index_to_char(sess.run(predict, feed_dict={X: [input2]})[0]))
        result.append(index_to_char(sess.run(predict, feed_dict={X: [input3]})[0]))
        result.append(index_to_char(sess.run(predict, feed_dict={X: [input4]})[0]))
...
if __name__ == '__main__':
    # 调用模型测试验证码
    test_image = 'test.jpg'
    print get_model_result(test_image)
